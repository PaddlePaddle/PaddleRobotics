...
class MujocoAgent(parl.Agent):
    def __init__(self, algorithm,alg_mode="SAC"):
    ...
    def predict(self, obs):
        ...
        # print(self.alg.predict(obs))
        action = self.alg.predict(obs)[0]
        # print(action)
    ...
    def BClearn(self, obs, ref_obs,ref_agent):
        obs = torch.FloatTensor(obs).to(self.device)
        ref_obs = torch.FloatTensor(ref_obs).to(self.device)
        critic_loss,actor_loss = self.alg.BClearn(obs, ref_obs,ref_agent)
        return critic_loss,actor_loss
