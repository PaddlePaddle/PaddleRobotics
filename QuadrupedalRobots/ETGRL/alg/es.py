...
class CMAES:
  ...
  def __init__(self, num_params,      # number of model parameters
               ...
               weight_decay=0.01,
               param = None,):    # weight decay coefficient
    ...
    import cma
    if param is None:
      param_init = self.num_params * [0]
    else:
      param_init = param
    self.es = cma.CMAEvolutionStrategy( param_init,
                                        self.sigma_init,
...
class SimpleES:
  def __init__(self,num_params,popsize=256,
                sigma_init=0.1,sigma_decay=0.999,
                sigma_limit=0.01,weight_decay=0.01,param=None):
  ...
  def tell(self, reward_table_result):
    ...
    elif self.curr_best_reward > self.best_reward:
        self.best_mu = best_mu
        self.best_reward = self.curr_best_reward
    ...
    dis = max_r - min_r
    if dis > 1e-2:
      reward = 3*(reward-min_r)/dis
    exp_reward = np.exp(reward)
    Sk = exp_reward/np.sum(exp_reward)
    self.mu = np.zeros(self.num_params)
    for i in range(self.popsize):
      self.mu += Sk[i]*self.solutions[i]
...
class SimpleGA:
  ...
  def result(self): # return best params so far, along with historically best reward, curr reward, sigma
    return (self.best_param, self.best_reward, self.curr_best_reward, self.sigma,self.curr_best_param)
